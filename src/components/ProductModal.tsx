...
export default function ProductModal({
  ...
  const [dispatchSeconds, setDispatchSeconds] = useState(2 * 60 * 60 + 45 * 60);
  ...
  // Live social-proof viewer counter (14 engineers baseline, gentle drift)
  useEffect(() => {
    const viewerInterval = setInterval(() => {
      setViewerCount((count) => {
        const next = count + (Math.random() > 0.5 ? 1 : -1);
        return Math.min(27, Math.max(9, next));
      });
    }, 6500);
    return () => clearInterval(viewerInterval);
  }, []);

  // Guaranteed same-day dispatch countdown timer
  useEffect(() => {
    const dispatchInterval = setInterval(() => {
      setDispatchSeconds((seconds) =>
        seconds <= 0 ? 2 * 60 * 60 + 45 * 60 : seconds - 1
      );
    }, 1000);
  ...
  const dispatchHours = Math.floor(dispatchSeconds / 3600);
  const dispatchMinutes = Math.floor((dispatchSeconds % 3600) / 60);
  const countdownLabel = `${dispatchHours}h ${String(dispatchMinutes).padStart(
    2,
    "0"
  )}m`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-6 flex flex-col justify-between space-y-6">
              <div>
                ...
                {/* LIVE URGENCY & SCARCITY BOX */}
                <div className="mt-3.5 rounded-xl border border-amber-500/40 bg-gradient-to-br from-amber-950/40 via-zinc-900/80 to-zinc-900/80 p-3.5 space-y-2.5 shadow-inner shadow-amber-500/5">
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-zinc-300">
                    ...
                    <span>
                      {viewerCount} engineers viewing right now
                    </span>
                  </div>

                  <div className="flex items-start gap-2 text-[11px] font-mono text-amber-300/90">
              ...
              <div className="pt-4 border-t border-zinc-800 space-y-3">
                ...
                <div className="mt-3 rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 space-y-2.5">
                  <div className="flex items-center gap-1.5 text-[10px] font-mono text-zinc-500 uppercase tracking-wider font-semibold">
                    <Lock className="w-3 h-3 text-emerald-400" />
                    <span>Guaranteed Safe & Secure Checkout</span>
                  </div>

                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="px-2 py-1 rounded-md bg-zinc-950 border border-zinc-700 text-[10px] font-mono font-semibold text-zinc-200 flex items-center gap-1">
                      <CreditCard className="w-3 h-3 text-zinc-400" />
                      Apple Pay
                    </span>
                    <span className="px-2 py-1 rounded-md bg-zinc-950 border border-zinc-700 text-[10px] font-mono font-semibold text-zinc-200">
                      Google Pay
                    </span>
                    <span className="px-2 py-1 rounded-md bg-zinc-950 border border-zinc-700 text-[10px] font-mono font-semibold text-zinc-200">
                      Shop Pay
                  ...
                  <div className="space-y-1.5 pt-2 border-t border-zinc-800">
                    <div className="flex items-center gap-2 text-[11px] font-mono text-zinc-300">
                      ...
                      <span>30-Day Zero-Risk Return Policy</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] font-mono text-zinc-300">
                      <BadgeCheck className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                      <span>100% Insured Priority Tracking</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] font-mono text-zinc-300">
                      <BadgeCheck className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
